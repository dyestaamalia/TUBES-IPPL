...
<div class="">
    ...
    <form action="{{ route('pets.update', $pet->id) }}" method="POST" enctype="multipart/form-data" class="mt-6 space-y-8">
        ...
        <div class="bg-white border rounded-xl p-6 shadow-sm">
            <div class="flex flex-col items-center">
                <div class="mb-4">
                    ...
                    @else
                        <div id="preview-image" class="w-32 h-32 rounded-full bg-gray-200 flex items-center justify-center text-4xl">
                            🐾
                        </div>
                    @endif
                </div>
                ...
                <label class="cursor-pointer w-full">
                    <div class="w-full border-2 border-dashed border-gray-300 rounded-xl py-10 flex flex-col items-center hover:bg-gray-50 transition">
                        ...
                        <p class="font-semibold">Upload Foto Baru</p>
        ...
        <div class="bg-white border rounded-xl p-6 shadow-sm">
            ...
            <textarea name="special_marks" rows="3" class="w-full p-3 border rounded-xl mt-1">{{ old('special_marks', $pet->special_marks) }}</textarea>
        </div>
        ...
        <div class="bg-white border rounded-xl p-6 shadow-sm">
            ...
            <select name="is_steril" class="w-full p-3 border rounded-xl mt-1">
                <option value="1" {{ $pet->is_steril ? 'selected' : '' }}>Sudah</option>
                <option value="0" {{ !$pet->is_steril ? 'selected' : '' }}>Belum</option>
            </select>
            ...
            <div class="mt-4">
                ...
                <textarea name="health_notes" rows="3" class="w-full p-3 border rounded-xl mt-1">{{ old('health_notes', $pet->health_notes) }}</textarea>
            </div>
        </div>

        <!-- Buttons -->
        <div class="flex justify-end gap-4">
            <a href="{{ route('hewan-saya') }}" class="px-6 py-2 border rounded-xl text-gray-600 hover:bg-gray-100">
                Batal
            </a>
...
function previewImage(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            preview.classList.remove('text-4xl');
            preview.textContent = '';
        }
